...
using Autumn.Authentication.TwoFactor.Google;
using Autumn.Authorization;
using Autumn.Authorization.Accounts.Dto;
...
using Autumn.Web.Models.TokenAuth;
using Autumn.Authorization.Impersonation;
using Autumn.Identity;
using Autumn.Net.Sms;
using Autumn.Notifications;
using Autumn.Web.Authentication.External;

namespace Autumn.Web.Controllers
{
    [Route("api/[controller]/[action]")]
    public class TokenAuthController : AutumnControllerBase
    {
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            var getUserRoles = await _userManager.GetRolesAsync(loginResult.User);

            if (!getUserRoles.Any(r => r == "Admin"))
            {
                if (loginResult.User.RegistrationStep != 5)
                {
