...
export class ExternalOnBoardProvider extends ExternalLoginProviderInfoModel {
    ...
    constructor(providerInfo: ExternalLoginProviderInfoModel) {
        ...
        this.icon = ExternalOnBoardProvider.getSocialIcon(this.name);
    }
...
export class OnboardService {
    ...
    externalLoginProviders: ExternalOnBoardProvider[] = [];
    rememberMe: boolean;
    ...
    authenticate(finallyCallback?: () => void, redirectUrl?: string): void {
        ...
        this.authenticateModel.twoFactorRememberClientToken = this._utilsService.getCookieValue(OnboardService.twoFactorRememberClientTokenName);
    ...
    externalAuthenticate(provider: ExternalOnBoardProvider): void {
        this.ensureExternalLoginProviderInitialized(provider, () => {
            if (provider.name === ExternalOnBoardProvider.FACEBOOK) {
                FB.login(response => {
                    this.facebookLoginStatusChangeCallback(response);
                }, { scope: 'email' });
            } else if (provider.name === ExternalOnBoardProvider.GOOGLE) {
                gapi.auth2.getAuthInstance().signIn().then(() => {
                    this.googleLoginStatusChangeCallback(gapi.auth2.getAuthInstance().isSignedIn.get());
                });
            } else if (provider.name === ExternalOnBoardProvider.MICROSOFT) {
                WL.login({
    ...
    private login(accessToken: string, encryptedAccessToken: string, expireInSeconds: number, rememberMe?: boolean, twoFactorRememberClientToken?: string, redirectUrl?: string): void {
        ...
        if (twoFactorRememberClientToken) {
            this._utilsService.setCookieValue(
                OnboardService.twoFactorRememberClientTokenName,
    ...
    private initExternalLoginProviders(callback?: any) {
        this._tokenAuthService
            ...
            .subscribe((providers: ExternalLoginProviderInfoModel[]) => {
                this.externalLoginProviders = _.map(providers, p => new ExternalOnBoardProvider(p));
    ...
    ensureExternalLoginProviderInitialized(loginProvider: ExternalOnBoardProvider, callback: () => void) {
        if (loginProvider.initialized) {
            callback();
            return;
        }

        if (loginProvider.name === ExternalOnBoardProvider.FACEBOOK) {
        ...
        } else if (loginProvider.name === ExternalOnBoardProvider.GOOGLE) {
        ...
        } else if (loginProvider.name === ExternalOnBoardProvider.MICROSOFT) {
        ...
        } else if (loginProvider.name === ExternalOnBoardProvider.OPENID) {
            const authConfig = this.getOpenIdConnectConfig(loginProvider);
            this.oauthService.configure(authConfig);
            this.oauthService.initImplicitFlow('openIdConnect=1');
        } else if (loginProvider.name === ExternalOnBoardProvider.WSFEDERATION) {
    ...
    private getWsFederationConnectConfig(loginProvider: ExternalOnBoardProvider): any {
        let config = {
            clientId: loginProvider.clientId,
            popUp: true,
    ...
    private getOpenIdConnectConfig(loginProvider: ExternalOnBoardProvider): AuthConfig {
        ...
        authConfig.redirectUri = window.location.origin + '/account/home';
    ...
    private facebookLoginStatusChangeCallback(resp) {
        if (resp.status === 'connected') {
            ...
            model.authProvider = ExternalOnBoardProvider.FACEBOOK;
    ...
    public openIdConnectLoginCallback(resp) {
        this.initExternalLoginProviders(() => {
            ...
            this.oauthService.tryLogin().then(() => {
                ...
                const model = new ExternalAuthenticateModel();
                model.authProvider = ExternalOnBoardProvider.OPENID;
    ...
    private googleLoginStatusChangeCallback(isSignedIn) {
        if (isSignedIn) {
            const model = new ExternalAuthenticateModel();
            model.authProvider = ExternalOnBoardProvider.GOOGLE;
    ...
    public wsFederationLoginStatusChangeCallback(errorDesc, token, error, tokenType) {
        ...
        const model = new ExternalAuthenticateModel();
        model.authProvider = ExternalOnBoardProvider.WSFEDERATION;
    ...
    */
    private microsoftLogin() {
        ...
        const model = new ExternalAuthenticateModel();
        model.authProvider = ExternalOnBoardProvider.MICROSOFT;
