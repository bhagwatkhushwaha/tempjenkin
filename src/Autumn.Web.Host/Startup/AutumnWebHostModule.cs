...
using Autumn.EmailServices;

namespace Autumn.Web.Startup
{
    [DependsOn(
        typeof(AutumnWebCoreModule)
    )]
    public class AutumnWebHostModule : AbpModule
    {
        private readonly IHostingEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public AutumnWebHostModule(
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(AutumnWebHostModule).GetAssembly());

            //hangfire background service
            IocManager.IocContainer.Register(
                Component.For<IAutumnBackgroundService>().Instance(new HangfireService())
                );

            //Amazon ses for emails
            IocManager.IocContainer.Register(
              Component.For<IAwsEmailSender>().Instance(new AmazonSESEmailSender())
              );
        }
